...
Администраторам (см. `ADMIN_IDS`) доступны команды управления кодами,
просмотра расхода токенов и выгрузки активности студентов.

Этот README описывает текущую реализованную версию. Подробная спецификация
сценария по PDF и фидбэку заказчиков записана в `materials/workflow-new.md`. Видео урока
и заранее подготовленные контексты вопросов уже находятся в проекте; контексты
ещё должны пройти экспертную проверку.
...
cmd/import-questions/  утилита импорта materials/question_bank.csv в БД
...
materials/              документация, исходный PDF и банки вопросов
  question_bank.csv     активный банк из 80 вопросов
  question_bank_*.csv   архивные версии банка с датами действия
```
...
Банк автоматически синхронизируется из `materials/question_bank.csv` при каждом старте
бота до запуска long polling. Команда `make questions` остаётся для ручной
синхронизации без запуска бота. CSV является источником истины: отсутствующие в
файле старые вопросы остаются в PostgreSQL для истории, но помечаются
неактивными и больше не выдаются новым сессиям.

## Администратор в Telegram

Администратор определяется только по Telegram ID из `ADMIN_IDS`. Секретного
слова в чате нет: его можно переслать или случайно раскрыть. Узнать собственный
Telegram ID можно через любого служебного бота для показа user ID, затем записать
его в `.env`, например `ADMIN_IDS=123456789`, и перезапустить приложение.

Команды:

- `/admin` — справка;
- `/code` или `/code 5` — создать от 1 до 20 одноразовых кодов;
- `/codes` — показать все выданные коды: сначала свободные, затем занятые с пользователем и расходом токенов;
- `/delete_code SA2026-XXXX` — удалить только неиспользованный код;
- `/tokens SA2026-XXXX` — расход входных, выходных, общих и кэшированных токенов пользователя;
- `/report` — общий отчёт по студентам с кодами доступа и расходом токенов.

Коды и статистика токенов хранятся в той же PostgreSQL, что профили и сессии.
Отдельная база не используется, чтобы связь `код → пользователь → сессия → вызов LLM`
оставалась целостной. Учёт токенов начинается с миграции `00016`; старые вызовы,
для которых раньше сохранялись только логи, задним числом восстановить нельзя.
...
пользователем и только скомпилированным бинарником + `prompts/` + `media/` + активным
`materials/question_bank.csv`. SQL-миграции
...
## Обслуживание production-сервера

Ниже описана текущая VPS-установка проекта в `/opt/sa-hr-bot`. Вместо
`IP_СЕРВЕРА` нужно подставить настоящий адрес сервера. Сам адрес, пароли,
Telegram-токен и OpenAI-ключ не следует сохранять в Git.

На сервере должны уже находиться локальные production-файлы
`compose.prod.yaml` и `prod.env`. Они содержат настройки конкретного сервера и
не заменяются командами из этого раздела.

### Обновить бота после изменений в `main`

Сначала изменения должны быть отправлены в GitHub. Затем:
...
`up -d --build bot` пересобирает образ только при необходимости и пересоздаёт
контейнер, если его конфигурация или образ изменились. При старте бот сам
применяет миграции и синхронизирует активный банк из
`materials/question_bank.csv`, поэтому отдельный импорт после обычного
обновления не нужен.

Обновление успешно, если контейнер имеет статус `Up`, а в логах есть
`question bank synchronized` с `active_questions=80`, затем `bot starting`, и
после них нет ошибки завершения процесса.

Если обычная пересборка явно использовала повреждённый или устаревший кэш,
выполнить аварийный вариант:
...
Этот вариант медленнее, поэтому для каждого обновления он не нужен.
...
Из просмотра непрерывных логов можно выйти через `Ctrl+C`: контейнер бота при
этом продолжит работать.

### Добавить администратора

Администраторы определяются только по цифровым Telegram ID. На сервере открыть
`prod.env`:

```bash
cd /opt/sa-hr-bot
TERM=xterm-256color nano prod.env
```

Добавить ID через запятую без пробелов:

```dotenv
ADMIN_IDS=123456789,987654321
```

После сохранения пересоздать контейнер, чтобы бот перечитал окружение:
...
С аккаунта нового администратора отправить `/admin`. Эта команда показывает
справку; отдельной команды `/commands` в текущей версии нет. Неавторизованным
пользователям админские команды ничего не отвечают.

Доступные действия администратора:

- `/code` или `/code 5` — выпустить от 1 до 20 одноразовых кодов;
- `/codes` — показать свободные и занятые коды, пользователей и расход токенов;
- `/delete_code SA2026-XXXX` — удалить неиспользованный код;
- `/tokens SA2026-XXXX` — посмотреть расход токенов конкретного пользователя;
- `/report` — получить общий отчёт по пользователям, кодам и токенам.

Если Telegram временно недоступен, коды можно создать с сервера:

```bash
cd /opt/sa-hr-bot
docker compose -f compose.prod.yaml --profile tools run --rm tools \
  go run ./cmd/generate-codes -n 5
```

Использованный код удалить нельзя: он связывает пользователя с историей и
отчётами. Полноценного отзыва доступа или блокировки зарегистрированного
пользователя в текущей версии ещё нет, поэтому использованные коды вручную из
PostgreSQL удалять не следует.
