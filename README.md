...
сам ведёт кандидата по фазам собеседования (инструкция, квалификация,
мини-аудит слабых зон, циклы вопросов из банка с оценкой ответа, итоговый
отчёт), опираясь на `internal/llm` и системный промпт в `prompts/`.
Администраторам (см. `ADMIN_IDS`) доступна команда `/report` — выгрузка
активности студентов.

Этот README описывает текущую реализованную версию. Согласованный сценарий
следующей версии по PDF и фидбэку заказчиков записан в `workflow-new.md`; он пока
находится на стадии планирования и ещё не реализован в коде.
...
пользователем и только скомпилированным бинарником + `prompts/`. SQL-миграции
