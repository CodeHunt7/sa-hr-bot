...
func writeEvaluationGuidelines(b *strings.Builder, profile StudentProfile) {
	...
	b.WriteString("- Обращайся к кандидату только на `вы`. Не используй `ты`, `тебе`, `твой` и формы прошедшего времени, указывающие на пол кандидата. Пиши: `вы указали`, `вы ответили`, `в вашем ответе`.\n")
	...
	b.WriteString("- Перед выводом сверь каждое утверждение с репликой кандидата. Не объявляй отсутствующим то, что он уже назвал напрямую или по смыслу.\n")
	b.WriteString("- Назови 1-3 конкретных сильных элемента из ответа. Если есть существенный пробел, выбери один и дай посильную подсказку. Если ответ закрывает вопрос на целевом уровне, прямо напиши: `Существенного пробела по этому вопросу нет`. Не выдумывай недостаток ради формата.\n")
...
func BuildEvaluationContext(profile StudentProfile, weakZones []db.WeakZone, studentAnswer string, question *db.QuestionBank) string {
	...
	b.WriteString("Верни только одну рамку `▸ ОБРАТНАЯ СВЯЗЬ` и 2-4 предложения внутри неё. Существенный пробел указывай только если он действительно есть; сильный ответ разрешено принять без критики. Не задавай следующий вопрос и не начинай новую фазу: уточнение из банка вопросов отправит код.\n")
...
func BuildFollowupEvaluationContext(profile StudentProfile, weakZones []db.WeakZone, primaryAnswer, followupQuestion, followupAnswer string, question *db.QuestionBank) string {
	...
	b.WriteString("Верни только одну рамку `▸ ОБРАТНАЯ СВЯЗЬ` и 2-4 предложения внутри неё. Существенный пробел указывай только если он действительно есть; сильный ответ разрешено принять без критики.\n")
...
func BuildBlockEvaluationContext(profile StudentProfile, weakZones []db.WeakZone, primaryAnswer, followup1Question, followup1Answer, followup2Question, followup2Answer string, question *db.QuestionBank) string {
	...
	b.WriteString("Если связка уже соответствует целевому уровню, в поле `Главная точка роста` прямо укажи, что существенной точки роста по содержанию нет. Не создавай искусственный недостаток.\n")
