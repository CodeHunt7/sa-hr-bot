...
- Одноразовый код даёт доступ на два календарных месяца с момента первого использования. После окончания срока данные сохраняются, а новый код продлевает доступ и возвращает пользователя к текущему шагу.
...
Одноразовый код действует два календарных месяца с момента первого использования. После истечения срока бот блокирует пользовательские команды, сообщения и кнопки, но не удаляет профиль, сессии и ответы. Команда `/start НОВЫЙ_КОД` активирует новый свободный код ещё на два месяца и продолжает сохранённую сессию с текущего шага. Админские команды сроком пользовательского доступа не блокируются.
...
> Привет. Меня зовут Катя Желатинка, я собрала для вас тренажёр для отработки технических собеседований системного аналитика.
>
...
Текст: `Какой у вас текущий грейд?`
...
Текст: `На какой грейд вы собеседуетесь?`
...
*выберите варианты и напишите мне их текстом*
```
...
```text
Зафиксировала ваши ответы. Правильно ли я понимаю?
...
В активном файле `question_bank.csv` находится 58 основных вопросов из таблицы Кати. Вместе со 116 используемыми уточнениями это 174 вопроса. Они распределены по шести внутренним темам: архитектура, интеграции, базы данных, требования, безопасность и `soft-skills`. Техническая самооценка по-прежнему использует пять прежних категорий; `soft-skills` доступны при случайном выборе и при углублении в уже выпавшую тему.
...
`SESSION_CYCLE_LIMIT` остаётся страховочным порогом. После достижения порога бот не завершает сессию молча, а спрашивает: `Хотите продолжить тренировку?` и показывает кнопки `Продолжить` и `Завершить тренировку`. При продолжении начинается новый блок и следующий такой контроль выполняется ещё через установленное число полных блоков.
...
Администраторы задаются списком Telegram ID в `ADMIN_IDS`. Команды `/code`, `/codes`, `/delete_code`, `/tokens` и `/report` недоступны всем остальным пользователям. `/codes`, `/tokens` и `/report` показывают даты и текущий статус доступа. Коды, пользователи, сессии и токены хранятся в одной PostgreSQL. Использованный код удалить нельзя, поскольку он является связью с профилем и историей тренировок.
