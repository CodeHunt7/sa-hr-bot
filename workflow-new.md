...
В рабочем банке 32 вопроса только по пяти утверждённым темам. Вопросы категории `подача` удалены. Также исключены низкоуровневые блоки про Kafka offset, God Object в наборе архитектурных паттернов, zero-downtime миграции схемы БД и cache stampede: они проверяют реализацию разработчика или DBA сильнее, чем работу системного аналитика. Удалённые вопросы в существующей БД помечаются неактивными, чтобы старые попытки сохраняли исходный текст, но новые сессии их не получали.
